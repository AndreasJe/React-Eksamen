...
export const addEvent = (title: string, description: string, imgURL: string, location: string, timeStart:string, timeEnd:string, dateStart:string, dateEnd: string) => {
    return async (dispatch: any, getState: any) => {
         const idToken = getState().user.idToken
        ...
        if (!response.ok) {
            ...
            dispatch({ type: ADD_EVENT, payload: { id: data.name, title,description, imgURL, location, timeStart, timeEnd, dateStart, dateEnd } })
        }
    };
};
